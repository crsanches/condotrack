...
function descricaoPeriodicidade(tarefa: TarefaPeriodica): string {
  ...
  if (p.tipo === 'semanal') {
    const nomes = p.diasSemana.map(d => DIAS[d]).join(', ')
    return `Semanal — ${nomes}`
  }
  if (p.tipo === 'intervalo') {
    return `A cada ${p.diasIntervalo} dias`
  }
  if (p.tipo === 'mensal') {
    return `Todo dia ${p.diaDoMes}`
  }
  return '—'
}
...
export default function TarefasPage() {
  ...
  async function load() {
    ...
    try {
      ...
      // ordena: atrasada → vence_hoje → nunca_executada → em_dia
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="px-4 mt-4 grid grid-cols-4 gap-2">
          {(
            ...
            >
              ...
              <p className={`text-[10px] uppercase font-medium ${textCls}`}>
                {label}
              </p>
            </button>
          ))}
        ...
        {/* BOTÃO NOVA TAREFA (só síndico/subsíndico) */}
        {isSindico && (
